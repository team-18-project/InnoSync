...
class _ProfileCreationPageState extends State<ProfileCreationPage> {
      ...
      if (p['work_experiences'] is List) {
        for (final we in p['work_experiences']) {
          _workExperiences.add({
            ...
            'endDate': (we['end_date'] ?? '').toString().isNotEmpty ? we['end_date'].toString().substring(0, 10) : '',
  ...
  Future<void> _uploadResume() async {
    ...
    final request = http.MultipartRequest('POST', Uri.parse('${ApiService.baseUrl}/profile/resume'));
    request.headers['Authorization'] = 'Bearer $token';
    request.files.add(await http.MultipartFile.fromPath('resume', _resumeFile!.path));
  ...
  List<Map<String, dynamic>> _prepareWorkExperiences() {
    ...
    for (final exp in _workExperiences) {
      if ((exp['position'] ?? '').trim().isEmpty || (exp['company'] ?? '').trim().isEmpty || (exp['startDate'] ?? '').trim().isEmpty) {
      ...
      result.add({
        ...
        'end_date': (exp['endDate'] ?? '').trim().isEmpty ? null : exp['endDate'],
        'description': (exp['description'] ?? '').trim().isEmpty ? null : exp['description'],
      });
    }
    return result;
  }
